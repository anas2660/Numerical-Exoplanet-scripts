...
template <class T> class matrix {
 public:
  matrix(unsigned int nRows, unsigned int nCols)
    ...
    if (!nRows || !nCols) { throw range_error("invalid matrix size"); }
  }
  static matrix identity(unsigned int nSize) {
    matrix oResult(nSize, nSize);
    int nCount = 0;
    generate(oResult.m_oData.begin(), oResult.m_oData.end(),
  ...
  inline matrix transpose() {
    ...
    for (unsigned int r = 0; r < m_nRows; ++r)
      for (unsigned int c = 0; c < m_nCols; ++c) oResult(c, r) += (*this)(r, c);
    return oResult;
  }
 ...
 public:
  ...
  matrix<T> Solve(matrix<T> &oMatrix) {
    ...
    for (int i = nCols - 1; i >= 0; i--) {
      oS(0, i) = oQtM(i, 0);
      for (int j = i + 1; j < nCols; j++) { oS(0, i) -= oS(0, j) * m_oR(i, j); }
      oS(0, i) /= m_oR(i, i);
    }
    return oS;
  }
...
vector<T> polyfit(const vector<T> &oX, const vector<T> &oY, int nDegree) {
  ...
  // copy y matrix
  for (size_t i = 0; i < nCount; i++) { oYMatrix(i, 0) = oY[i]; }
  // create the X matrix
  ...
  return oCoeff.data();  // copy the result to coeff
}
...
vector<T> polyval(const vector<T> &oCoeff, const vector<T> &oX) {
  ...
  for (size_t i = 0; i < nCount; i++) {
    T nY = 0, nXT = 1, nX = oX[i];
    for (size_t j = 0; j < nDegree; j++) {
      nY += oCoeff[j] * nXT;  // multiply current x by a coefficient
      nXT *= nX;              // power up the X
    }
    oY[i] = nY;
  }
  return oY;
